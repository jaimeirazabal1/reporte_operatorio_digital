...
class Util
{
    ...
    public static function verNombreArchivoPdf($path_completo){
        $path = explode("/",$path_completo);
        $tamano = count($path);
        $nombre = $path[$tamano-1];
        return $nombre;
    }
    /**
    ...
    public static function sumarHoras($h1,$h2)
    {
        $h1 = explode(":",$h1);
        $h2 = explode(":",$h2);


        if (count($h1) > 1 and count($h2) > 1) {
    ...
    public static function getPermisos($file){
